...
class SavePlaceUseCaseTest {
    ...
    lateinit var useCaseTest: SavePlaceUseCase
    @Before
    fun setUp() {
        MockKAnnotations.init(this)
        mapper = PlaceDataMapperImpl()
        useCaseTest = SavePlaceUseCase(repository, UnconfinedTestDispatcher())
    }

    @Test
    fun execute() = runTest {
        val response = mapper.responseToDomain(PlaceRepositoryDataGenerator.detailResponse)
        coEvery { repository.saveFavoritePlace(response) } returns Result.Success(Unit)
    ...
    fun executeError() = runTest {
        val response = mapper.responseToDomain(PlaceRepositoryDataGenerator.detailResponse)
        coEvery { repository.saveFavoritePlace(response) } returns Result.Error(Exception("error"))
        useCaseTest(response).let {
            assert(it.isError)
        }
    }
